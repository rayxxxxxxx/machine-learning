...
def visualize(fa: FA):
    x = np.linspace(fa.lb, fa.ub, 50)
    y = np.linspace(fa.lb, fa.ub, 50)
    x, y = np.meshgrid(x, y)

    z = fa.optimizationFunction(x, y)
    ...
    def anim(t):
        fireFliesPositions = fa.getFirefliesPositions()
        scatter_x = [pos[0] for pos in fireFliesPositions]
        scatter_y = [pos[1] for pos in fireFliesPositions]
        scatter_z = [fa.optimizationFunction(*pos) for pos in fireFliesPositions]
